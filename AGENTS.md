...
- `game.js` (`'use strict'`, ~915 lines) — classes `Ship`, `Asteroid`, `Bullet`, `Particle`, `PowerUp` (`PowerUp` con `kind: speed|triple|shield` + `Asteroid` con `opts.shootingStar` para la Estrella Fugaz) + state machine `playing | dead | gameover` (`initGame` / `nextLevel` / `spawnAsteroids` / `spawnShootingStar` / `explode` / `killShip` / `update` / `draw` / `loop`) + skins de nave (`SHIP_SKINS` / `getSkin` / `setShipSkin` / `loadSkin` / `saveSkin` / `checkSkinInput` / `shipPath`).
